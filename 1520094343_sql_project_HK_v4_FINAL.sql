...
SELECT facid AS facility_id, name AS facility_name, membercost AS fee, monthlymaintenance AS maintenance
FROM Facilities
WHERE membercost >0
...
SELECT  CONCAT(Members.firstname, ' ', Members.surname) AS member_name,
        CASE WHEN Bookings.facid = 0 THEN 'Tennis Court 1'
             WHEN Bookings.facid = 1 THEN 'Tennis Court 2'
                ELSE 'No_Tennis_Court' END AS Tennis_Court
     FROM Members
     JOIN Bookings
...
SELECT Bookings.bookid,
        ...
        JOIN Members
            ...
            HAVING cost > 30
            ORDER BY cost DESC
/* Note - Unable to apply cost > 30 filter to the results*/
...
SELECT sub.fac_name as facilities_name,
